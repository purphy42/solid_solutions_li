...
Current training defaults (see [`model_train.py`](model_train.py) / [`data/model_train_results_opt.csv`](data/model_train_results_opt.csv)):

- Hyperparameter search: **randomized** (`HYPERPARAM_SEARCH='random'`)
- Model selection: minimal **cross-validation** RMSE (`SELECT_BEST_BY='cv'`)
- Features: significance + correlation filtering, capped at 25
- Metrics and figures are written with `_pre` / `_opt` tags (`METRICS_SUFFIX`)
...
|------|-----------|----------------------|
| **1. Atomic size** | Relative radius mismatch $\|r_X-r_{\mathrm{Li}}\| / \max(r_X,r_{\mathrm{Li}}) \le 15\%$ | Metallic / XRD radii in pm (`r_atomic_pm`) |
...
| **4. Electronegativity** | $\|\chi_X-\chi_{\mathrm{Li}}\|\le 0.4$ (Pauling) | Absolute $\Delta\chi$ (also stored as %) |

Outputs:

- Rule-enriched table: [`figures/h_r_rules/elements_data.csv`](figures/h_r_rules/elements_data.csv)
...
Suffixes `_pre` / `_opt` mark the energy pool used to build or evaluate that table.
...
| File | Description |
|------|-------------|
| [`binding_feature_importance_{pre,opt}.csv`](data/binding_feature_importance_opt.csv) | RF MDI + permutation importance for $E_b$ |
| [`solution_feature_importance_{pre,opt}.csv`](data/solution_feature_importance_opt.csv) | Same for $E_s$ |
| [`solution_minus_binding_feature_importance_{pre,opt}.csv`](data/solution_minus_binding_feature_importance_opt.csv) | Same for $E_s-E_b$ |
...
| [`model_train_results_{pre,opt}.csv`](data/model_train_results_opt.csv) | Full training summary (hyperparams, metrics, feature counts) |
| [`derived_effective_energies_opt.csv`](data/derived_effective_energies_opt.csv) | Per-dopant DFT vs ML $E_s$, $E_b$, piecewise $E_{\mathrm{eff}}$ (written for the active `METRICS_SUFFIX` run) |
